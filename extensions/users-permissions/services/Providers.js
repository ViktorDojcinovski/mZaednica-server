...
const connect = (ctx, provider, query) => {
...
const getProfile = async (provider, query, callback) => {
  ...
  switch (provider) {
    ...
    case "apple": {
      const appleUserInfo = await strapi.plugins.apple.services.apple.claim(
        access_token,
        query.useBundleId
      );
      ...
      callback(
        null,
        {
          username: appleUserInfo.email.split("@")[0],
...
const buildRedirectUri = (provider = "") => {
  if (provider === 'apple') {
    return `${getAbsoluteServerUrl(strapi.config)}/${provider}/callback`;
  } else {
    return `${getAbsoluteServerUrl(strapi.config)}/connect/${provider}/callback`;
  }
}
